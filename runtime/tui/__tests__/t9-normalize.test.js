...
test("T9.1: 42 skills reais carregadas com rawCategory verbatim do manifesto", () => {
  const views = normalizeSkills(fixtureRegistry());
  const maestro = views.filter((v) => v.namespace === "maestro");
  assert.equal(maestro.length, 42, "manifesto real tem 42 skills");
...
test("T9.1: contagem normalizada bate com o mapeamento oficial do plano (5/3/4/8/5/17=42)", () => {
  ...
  const expectation = { Development: 5, Frontend: 3, Backend: 4, Security: 8, Automation: 5, Other: 17 };
